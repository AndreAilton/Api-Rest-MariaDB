...
class FileController {
  store(req, res) {
    ...
    return upload(req, res, async (error) => {
      ...
      try {
        ...
        const File = await file.create({ originalname, filename, user_id, category });

        if(req.body.Category) {
          if (req.body.Category) {

            const userDir = resolve(__dirname, '..', '..', 'uploads', 'images', `${req.userId}`,'nocategory', filename)
 ...
 async delete(req, res) {
    try {
      ...
      const BackupDir = resolve(__dirname, '..', '..', 'uploads', 'images', `${req.userId}`,'Backup')
      const categoryDir = resolve(__dirname, '..', '..', 'uploads', 'images', `${req.userId}`, `${File.category}`);
      ...
      await File.save();
      return res.status(200).json({sucess:true, message: 'Foto deletada com sucesso'});
  ...
  async update (req, res) {
    try {
      ...
      if (req.body.category === File.category) {
        ...
        return res.status(200).json({sucess:true, message: 'Foto Alterada com sucesso'});

      }
      const oldcategorydir = resolve(__dirname, '..', '..', 'uploads', 'images', `${req.userId}`, `${File.category}`);
      const oldFilePath = resolve(oldcategorydir, File.filename);

      File.update(req.body);

      const newcategoryDir = resolve(__dirname, '..', '..', 'uploads', 'images', `${req.userId}`, `${File.category}`);
